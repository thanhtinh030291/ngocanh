...
function format_curency($val){
	$curency_code = session()->get('curency_codelocale');
	$result = "";
	if($curency_code == "VND"){
	...
	return $result." ".session()->get('currencylocale');
}


function waterMarkImage($value){
    // $img = Image::make(public_path($value)); //your image I assume you have in public directory
    // $img->insert(public_path('img/logoNT7solution-watermark.png'), 'bottom-right', 10, 10); //insert watermark in (also from public_directory)
